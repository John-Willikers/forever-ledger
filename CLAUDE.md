...
| Item spec info populated? | `C_Item.GetItemSpecInfo` and `C_Item.DoesItemContainSpec` exist in the docs. **Unknown** whether Forever fills them (own class only / all classes / always empty): run `/flprobe specs` (probe 0.3.0) and record it here.                 |
| Spec catalog per class    | `C_SpecializationInfo.GetNumSpecializationsForClassID` / `GetSpecializationInfo(…, classID)` (role, `primaryStat`) and global `GetSpecializationInfoForClassID` exist. **Unknown** ids / roles / stats on Forever: `/flprobe specs`.      |
